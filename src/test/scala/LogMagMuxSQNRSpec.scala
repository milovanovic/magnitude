...
class SqrMagSQNRSpec extends AnyFlatSpec with Matchers {
  ...
  dsptools.Driver.execute(() => new LogMagMuxGenerator(paramsFixed16), Array("-tbn", "treadle")) { c =>
    new LogMagMuxTester(c) {
      var count = 0
      while (count < numSamples) {
        val real = Seq.fill(inputDataSize)(
          (Random.nextInt(
            (1 << (paramsFixed16.protoIn.getWidth - 1)) * 2
          ) - (1 << (paramsFixed16.protoIn.getWidth - 1))).toDouble / (pow(2, inputBinaryPoint))
        )
        // print real
        val imag = Seq.fill(inputDataSize)(
          (Random.nextInt(
            (1 << (paramsFixed16.protoIn.getWidth - 1)) * 2
          ) - (1 << (paramsFixed16.protoIn.getWidth - 1))).toDouble / (pow(2, inputBinaryPoint))
        )
  ...
  binaryPointGrowth = 0
  trimNum =
    if (((dataWidthIn - inputBinaryPoint) * 2 + 1 + inputBinaryPoint + binaryPointGrowth) - dataWidthOut > 0)
  ...
  dsptools.Driver.execute(() => new LogMagMuxGenerator(paramsFixed24), Array("-tbn", "treadle")) { c =>
    new LogMagMuxTester(c) {
      var count = 0
      while (count < numSamples) {
        val real = Seq.fill(inputDataSize)(
          (Random.nextInt(
            (1 << (paramsFixed24.protoIn.getWidth - 1)) * 2
          ) - (1 << (paramsFixed24.protoIn.getWidth - 1))).toDouble / (pow(2, inputBinaryPoint))
        )
        val imag = Seq.fill(inputDataSize)(
          (Random.nextInt(
            (1 << (paramsFixed24.protoIn.getWidth - 1)) * 2
  ...
  dsptools.Driver.execute(
    ...
    () => new LogMagMuxGenerator(paramsFixed32),
    Array("-tbn", "verilator")
  ) { c =>
    new LogMagMuxTester(c) {
      var count = 0
      while (count < numSamples) {
        val real = Seq.fill(inputDataSize)(
          (Random.nextInt((1 << (dataWidthIn - 1)) * 2) - (1 << (dataWidthIn - 1))).toDouble / (pow(
            2,
            inputBinaryPoint
          ))
        )
