...
abstract class LogMagMuxBlock [T <: Data : Real: BinaryRepresentation, D, U, E, O, B <: Data] (params: MAGParams[T], beatBytes: Int) extends LazyModule()(Parameters.empty) with DspBlock[D, U, E, O, B] with HasCSR {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val logMagMux = Module(new LogMagMux(params))
    ...
    logMagMux.io.in.bits     := in.bits.data.asTypeOf(params.proto)
...
// import freechips.rocketchip.tilelink._
// 
// abstract class LogMagMuxBlock [T <: Data : Real: BinaryRepresentation, D, U, E, O, B <: Data] (params: MAGParams[T], beatBytes: Int, hasMemMap: Boolean) extends LazyModule()(Parameters.empty) with DspBlock[D, U, E, O, B] {
...
// 
// 
// class AXI4LogMagMuxBlock[T <: Data : Real: BinaryRepresentation](params: MAGParams[T], address: AddressSet, _beatBytes: Int = 4, hasAXI4: Boolean)(implicit p: Parameters) extends LogMagMuxBlock[T, AXI4MasterPortParameters, AXI4SlavePortParameters, AXI4EdgeParameters, AXI4EdgeParameters, AXI4Bundle](params, _beatBytes, hasAXI4) with AXI4DspBlock {
...
// 
// 
// class TLLogMagMuxBlock[T <: Data : Real: BinaryRepresentation](val params: MAGParams[T], address: AddressSet, beatBytes: Int = 4, hasTL: Boolean)(implicit p: Parameters) extends LogMagMuxBlock[T, TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLEdgeIn, TLBundle](params, beatBytes, hasTL) with TLDspBlock {
//   val devname = "TLLogMagMuxBlock"
//   val devcompat = Seq("LogMagMux", "radarDSP")
//   val device = new SimpleDevice(devname, devcompat) {
//     override def describe(resources: ResourceBindings): Description = {
...
//   implicit val p: Parameters = Parameters.empty
//   
//   val mag = LazyModule(new AXI4LogMagMuxBlock(params, AddressSet(baseAddress + 0x100, 0xFF), _beatBytes = 4, hasAXI4 = true) with dspblocks.AXI4StandaloneBlock {
//     override def standaloneParams = AXI4BundleParameters(addrBits = 32, dataBits = 32, idBits = 1)
//   })
...
//   implicit val p: Parameters = Parameters.empty
//   
//   val mag = LazyModule(new AXI4LogMagMuxBlock(params, AddressSet(baseAddress + 0x100, 0xFF), _beatBytes = 4, hasAXI4 = false) with dspblocks.AXI4StandaloneBlock {
//     override def standaloneParams = AXI4BundleParameters(addrBits = 32, dataBits = 32, idBits = 1)
//   })
//   chisel3.Driver.execute(args, ()=> mag.module) // generate verilog code  
// }
// 
// // and simple verilog test for Tilelink goes here ... 
// 
