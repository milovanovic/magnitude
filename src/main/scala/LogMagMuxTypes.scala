...
class MagJPLInst[T <: Data: Real: BinaryRepresentation](val params: MAGParams[T])
    extends Module
    with HasCommonInterface {
  ...
  val jplMagOp1 = DspContext.withNumAddPipes(2 * numAddPipes) { // to be aligned with jplMagOp2
    u.context_+(BinaryRepresentation[T].shr(v, 3))
  }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
class LogMagInst[T <: Data: Real: BinaryRepresentation](val params: MAGParams[T])
    extends Module
    with HasCommonInterface {
  val io = IO(MagMuxIO(params))
  ...
  val jplMagOp1 = DspContext.withNumAddPipes(2 * numAddPipes) {
    u.context_+(BinaryRepresentation[T].shr(v, 3))
  } // U + V/8
  val tmpOp2 = DspContext.withNumAddPipes(numAddPipes) { u.context_-(BinaryRepresentation[T].shr(u, 3)) }
  ...
  val logLookUp = VecInit({
    ...
    val num = 1 << (params.log2LookUpWidth)
    val lookup = (0 until num).map(n => {
      val lookupWire = Wire(FixedPoint((bposLog + 1).W, bposLog.BP))
      // log2(1+f)
  ...
  // playing with BRAM //
  /*val num = 1 << (params.log2LookUpWidth)
  val memSRM = SyncReadMem(num, params.protoOut.cloneType)
  val memoryFile = "test.hex"
  val w = new BufferedWriter(new FileWriter(memoryFile))

  for (i <- 0 until num) {
    val data_to_write = log2(i.toDouble / num + 1) * scala.math.pow(2, bposLog)
    w.write(f"${data_to_write.toInt}%02x" + "\n")
  }
  w.close()

  if (memoryFile.trim().nonEmpty) {
    loadMemoryFromFileInline(memSRM, memoryFile)
  }*/
  ...
  val logLookUpAddr =
    BinaryRepresentation[UInt].shl(noLeadOne, shiftNum)(dataWidth - 2, dataWidth - params.log2LookUpWidth - 1)
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
class MagJPLandSQRMagInst[T <: Data: Real: BinaryRepresentation](val params: MAGParams[T])
    extends Module
    with HasCommonInterface {
  val io = IO(MagMuxIO(params))
  ...
  val magSqr = DspContext.alter(
    DspContext.current
      .copy(numAddPipes = numAddPipes, numMulPipes = numMulPipes, binaryPointGrowth = params.binPointGrowth)
  ) {
    ShiftRegister(
      (absInReal.context_*(absInReal)).context_+(absInImag.context_*(absInImag)),
      latency - magSqrLatency,
      true.B
    )
  }
  dontTouch(magSqr)
  ...
  val trimMagSqr = DspContext.alter(DspContext.current.copy(trimType = params.trimType, binaryPointGrowth = 0)) {
    magSqr.div2(trimNum)
  }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
class MagJPLandLogMagInst[T <: Data: Real: BinaryRepresentation](val params: MAGParams[T])
    extends Module
    with HasCommonInterface {
  val io = IO(MagMuxIO(params))
...
/**
  ...
  *  and log2 of the magnitude. It can bypass input data as well.
  */
class LogMagMuxInst[T <: Data: Real: BinaryRepresentation](val params: MAGParams[T])
    extends Module
    with HasCommonInterface {
  val io = IO(MagMuxIO(params))
  ...
  val magSqr = DspContext.alter(
    DspContext.current.copy(numAddPipes = numAddPipes, numMulPipes = numMulPipes, binaryPointGrowth = 0)
  ) {
    ShiftRegister(
      (absInReal.context_*(absInReal)).context_+(absInImag.context_*(absInImag)),
      latency - magSqrLatency,
  ...
  val log2Mag = Wire(FixedPoint(dataWidthLog.W, bposLog.BP))
  log2Mag := ShiftRegister(
    ShiftRegister(logSInt.asFixedPoint(0.BP), numAddPipes, true.B) + logFrac,
    latency - log2Latency,
    true.B
  )
