...
object Skid {
  def apply[T <: Data](latency: Int, in: DecoupledIO[_ <: Data], out: DecoupledIO[T], initDone: Bool = true.B, en: Bool = true.B): T = {
    ...
    in.ready := out.ready || ~initDone //(queueCounter < latency.U) || (queueCounter === latency.U && out.ready)
